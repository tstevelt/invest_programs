...
#define DEBUG
	...
	else
	{
		...
		{
			sprintf ( WhereClause, "Fticker=Sticker and Stype='%c'", xstock.xstype[0] );
		...
		{
			if ( xstock.xstype[0] != STYPE_ETF && Array[ndx].isETF )
			{
				continue;
			}
			else if ( xstock.xstype[0] == STYPE_ETF && Array[ndx].isETF == 0 )
			{
				continue;
			}
